  ...
  class CLI < Thor
    ...
    def mint
      ...
      if metadata.creator[:id] == bot.client_id
        ...
        File.rename options[:metadata], [options[:metadata], metadata.metahash].join("_")
      else
        payment =
    ...
    def bulkmint(dir)
      ...
      Dir.glob("#{dir}/*.json").each do |file|
        ...
        next if file.split("-").last == metadata.metahash
        ...
        File.rename file, [file, metadata.metahash].join("-")
